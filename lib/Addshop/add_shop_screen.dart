...
class _AddShopScreenState extends State<AddShopScreen> {
  ...
  void selectBarberImage(BuildContext context) async {
    ...
    if (!mounted) return;
    File compressImage =
        await Provider.of<AddShopProvider>(context, listen: false)
  ...
  void uploadFile(context) async {
    ...
    try {
      final barberRef =
          FirebaseStorage.instance.ref().child(barberImageDestination);
      final shopRef = FirebaseStorage.instance.ref().child(shopDestination);
      final coverShopRef =
          FirebaseStorage.instance.ref().child(shopCoverDestination);
      ...
      final barberImageUrl =
          await snapshot1.ref.getDownloadURL().whenComplete(() {
        // Provider.of<LoadingProvider>(context,listen: false).startLoading();
      });
      final shopImageUrl =
          await snapshot2.ref.getDownloadURL().whenComplete(() {
        // Provider.of<LoadingProvider>(context,listen: false).startLoading();
      });
      final coverImageUrl =
      ...
      var userSnapShot = await FirebaseCollection()
          .userCollection
          .where('userEmail',
      ...
      for (var snapshot in userSnapShot.docChanges) {
        LoginProvider().addUserDetail(
            ...
            hairCategory: Provider.of<AddShopProvider>(context, listen: false)
                .selectHairStyle
                .toString(),
            ...
            gender: Provider.of<AddShopProvider>(context, listen: false)
                .selectGender
                .toString(),
            address: addressController.text,
            coverPageImage: coverImageUrl,
            barberImage: barberImageUrl,
  ...
  Future<void> selectOpeningTime(BuildContext context) async {
    ...
    if (!mounted) return;
    if (pickedOpeningTime != null &&
        pickedOpeningTime !=
            Provider.of<AddShopProvider>(context, listen: false)
                .selectedOpeningTime) {
      Provider.of<AddShopProvider>(context, listen: false).selectedOpeningTime =
          pickedOpeningTime!;
      setState(() {});
    }
  }
  ...
  Future<void> selectClosingTime(BuildContext context) async {
    ...
    if (!mounted) return;
    if (pickedClosingTime != null &&
        pickedClosingTime !=
            Provider.of<AddShopProvider>(context, listen: false)
                .selectedClosingTime) {
      Provider.of<AddShopProvider>(context, listen: false).selectedClosingTime =
          pickedClosingTime!;
      setState(() {});
    }
  }
              ...
              return Column(
                children: [
                  ...
                  DropdownButtonFormField2(
                    ...
                    hint: const Text('Select Hair Category',
                        style: TextStyle(
                            fontSize: 13, fontFamily: AppFont.regular)),
                    isExpanded: true,
                    isDense: true,
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            pickedOpeningTime == null
                                ? "Opening Hour"
                                : Provider.of<AddShopProvider>(context,
                                        listen: false)
                                    .selectedOpeningTime
                                    .format(context),
                            style: TextStyle(
                      ...
                      onTap: () {
                        if (_formKey.currentState!.validate()) {
                          if (barberFile != null &&
                            ...
                            } else if (Provider.of<AddShopProvider>(context,
                                            listen: false)
                                        .longitude ==
                                    '' &&
                                Provider.of<AddShopProvider>(context,
