  ...
  Widget build(BuildContext context) {
    ...
    void uploadFile() async {
      ...
      try {
        ...
        final imageUrl =
            await snapshot.ref.getDownloadURL().whenComplete(() {});

        var snapshotData = await FirebaseCollection()
            .userCollection
            .where('userEmail',
        ...
        for (var data in snapshotData.docChanges) {
          ...
          LoginProvider().addUserDetail(
              ...
              shopName: data.doc.get("shopName"),
              shopDescription: data.doc.get('shopDescription'),
              rating: data.doc.get('rating'),
              status: data.doc.get('shopStatus'),
              openingHour: data.doc.get('openingHour'),
              closingHour: data.doc.get('closingHour'),
              barberName: data.doc.get('barberName'),
              currentUser: data.doc.get('currentUser'),
              hairCategory: data.doc.get('hairCategory'),
              price: data.doc.get('price'),
              longitudeShop: data.doc.get('longitude'),
              latitudeShop: data.doc.get('latitude'),
              contactNumber: data.doc.get('contactNumber'),
              webSiteUrl: data.doc.get('webSiteUrl'),
              gender: data.doc.get('gender'),
              address: data.doc.get('address'),
              coverPageImage: data.doc.get('coverPageImage'),
              barberImage: data.doc.get('barberImage'),
              shopImage: data.doc.get('shopImage'),
              userType: data.doc.get('userType'),
              timestamp: data.doc.get('timeStamp'));
                      ...
                      child: Column(
                        children: [
                          ...
                          TextFieldMixin().textFieldWidget(
                            ...
                            prefixIcon: Image.network(
                                'https://cdn-icons-png.flaticon.com/128/9637/9637721.png',
                                scale: 5),
                                ...
                                if (formKey.currentState!.validate()) {
                                  if (file != null) {
                                    ...
                                    LoginProvider().addUserDetail(
                                        ...
                                        price: data['price'],
                                        longitudeShop: data['longitude'],
                                        latitudeShop: data['latitude'],
                                        contactNumber: data['contactNumber'],
                                        webSiteUrl: data['webSiteUrl'],
                                        gender: data['gender'],
                                        address: data['address'],
                                        coverPageImage: data['coverPageImage'],
                                    ...
                                    AppUtils.instance.showToast(
                                        toastMessage: 'Profile is updated');
                                  }
                                }
                              },
